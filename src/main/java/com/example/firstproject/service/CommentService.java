...
//        // 2. 엔티티 -> DTO 반환
//        List<CommentDto> dtos = new ArrayList<CommentDto>(); // 조회한 댓글 엔티티 수만큼 반복하기
//        for (int i = 0; i < comments.size(); i++) { // 조회한 댓글 엔티티 하나씩 가져오기
//            Comment c = comments.get(i); // 엔티티를 DTO로 변환
...
//        }
        ...
        return commentRepository.findByArticleId(articleId) // 댓글 엔티티 목록 조회
                ...
                .map(comment -> CommentDto.createCommentDto(comment)) // 엔티티르 DTO로 매핑
                .collect(Collectors.toList()); // 스트림을 리스트로 변환
    }
    @Transactional // 문제 발생시 롤백
